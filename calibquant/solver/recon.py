...
from calibquant.quantization.quantized_module import QuantizedModule
from calibquant.quantization.fake_quant import LSQFakeQuantize, QuantizeBase
...
def reconstruction(model, fp_model, module, fp_module, cali_data, config):
    ...
    quant_inp, _ = save_inp_oup_data(model, module, cali_data, store_inp=True,
                                     store_oup=False, bs=config.batch_size, keep_gpu=config.keep_gpu)
    fp_inp, fp_oup = save_inp_oup_data(fp_model, fp_module, cali_data, store_inp=True,
                                       store_oup=True, bs=config.batch_size, keep_gpu=config.keep_gpu)
    w_para, a_para = [], []
    ...
    if len(a_para) != 0:
        ...
        a_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
            a_opt, T_max=config.iters, eta_min=0.)
    else:
    ...
    for i in range(config.iters):
        ...
        if config.drop_prob < 1.0:
            ...
            cur_inp = torch.where(torch.rand_like(
                cur_quant_inp) < config.drop_prob, cur_quant_inp, cur_fp_inp)
        else:
    ...
    for name, layer in module.named_modules():
        if isinstance(layer, (nn.Linear, nn.Conv2d)):
            weight_quantizer = layer.weight_fake_quant
            layer.weight.data = weight_quantizer.get_hard_value(
                layer.weight.data)
