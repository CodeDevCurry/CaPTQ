...
def load_data(type: str = "imagenet", path: str = '', input_size: int = 224, batch_size: int = 64,
    ...
    # pin_memory是DataLoader的一个参数，用于指定是否将加载的数据存储在CUDA固定内存上。
    # 当pin_memory=True时，PyTorch会尝试将数据加载到固定的CUDA内存中，以避免在训练时因为数据传输延迟而等待。
    # 但是，如果主机系统的内存不足以容纳所有的数据，这可能会导致OutOfMemory错误。
    normalize = transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                     std=[0.229, 0.224, 0.225])
    traindir = os.path.join(path, 'sel_2')
    ...
    val_loader = torch.utils.data.DataLoader(
        ...
        num_workers=num_workers, pin_memory=pin_memory)
    logger.info('traindir: {}'.format(traindir))
    logger.info('valdir: {}'.format(valdir))
